...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then(async (cache) => {
      // App shell : chaque fichier est mis en cache individuellement.
      // Si l'un d'eux échoue (chemin erroné, fichier absent du déploiement),
      // les autres sont quand même mis en cache au lieu de tout faire échouer
      ...
      results.forEach((r, i) => {
        if (r.status === 'rejected') {
          console.warn('[Service Worker] Échec de mise en cache :', APP_SHELL[i], r.reason);
        }
      });
